...
本プロジェクトは Python パッケージの他に PortAudio / libsndfile / ffmpeg 等の OS レベルの依存が必要です。代表的なコマンド例（Linux 向け）を示します。macOS/Windows 固有のメモは現状リポジトリ未整備なので、必要に応じてプロジェクト Wiki や README に追記してください。
